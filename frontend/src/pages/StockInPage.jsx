...
useEffect(() => {
  ...
  const loadHistory = async (pageNum) => {
    try {
      const res = await stockInAPI.getHistory({ page: pageNum, limit: 10 });
                ...
                <Grid container spacing={2}>
                  ...
                  {formData.productId && (
                    <Grid item xs={12}>
                      <TextField
                        fullWidth
                        label={`Current Stock (${products.find(p => p._id === formData.productId)?.unitId?.code || 'units'})`}
                        value={currentStock}
                        disabled
                        size="small"
                        sx={{
                  ...
                  <Grid item xs={12}>
                    ...
                    >
                      {loading ? <CircularProgress size={24} color="inherit" /> : '✓ Save Stock In'}
                    </Button>
                  </Grid>
                </Grid>
              </form>

...
export const createStockOut = async (req, res, next) => {
  try {
    ...
    if (currentStock < quantity) {
      const unitCode = product.unitId ? product.unitId.code : 'units';
      return res.status(400).json({
        success: false,
        message: `Insufficient stock. Available: ${currentStock} ${unitCode}. Requested: ${quantity} ${unitCode}`
      });
    }
...
export const getStockOutHistory = async (req, res, next) => {
  try {
